...
function restructureChatBoxes() {
	...
	for (x in chatBoxes) {
		...
		if (jQuery("#chatbox_"+chatboxtitle).css('display') != 'none') {
			if (align == 0) {
				jQuery("#chatbox_"+chatboxtitle).css('right', '300px');
			} else {
				width = (align)*(245+7)+300;
...
function createChatBox(chatboxtitle,minimizeChatBox) {
	...
	.addClass("chatbox")
	.html('<div class="chatboxhead"><div class="chatboxtitle">'+chatboxtitle+'</div><div class="chatboxoptions"><a href="javascript:void(0)" onclick="javascript:toggleChatBoxGrowth(\''+chatboxtitle+'\')">&ndash;</a> <a href="javascript:void(0)" onclick="javascript:closeChatBox(\''+chatboxtitle+'\')">x</a></div><br clear="all"/></div><div class="chatboxcontent"></div><div class="chatboxinput"><textarea class="chatboxtextarea" onkeydown="javascript:return checkChatBoxInputKey(event,this,\''+chatboxtitle+'\');"></textarea></div>')
	...
	if (chatBoxeslength == 0) {
		...
		width = (chatBoxeslength)*(245+7)+300;
		jQuery("#chatbox_"+chatboxtitle).css('right', width+'px');
	}
...
function chatHeartbeat(){
	...
	jQuery.ajax({
	  url: BASE_URL+"chatty/chat.php?action=chatheartbeat",
	  cache: false,
	  dataType: "json",
...
function closeChatBox(chatboxtitle) {
	...
	jQuery.post( BASE_URL+"chatty/chat.php?action=closechat", { chatbox: chatboxtitle} , function(data){	
	});
...
function checkChatBoxInputKey(event,chatboxtextarea,chatboxtitle) {
	...
	if(event.keyCode == 13 && event.shiftKey == 0)  {
		...
		if (message != '') {
			jQuery.post( BASE_URL+"chatty/chat.php?action=sendchat", {to: chatboxtitle, message: message} , function(data){
...
function startChatSession(){  
	jQuery.ajax({
	  url:  BASE_URL+"chatty/chat.php?action=startchatsession",
	  cache: false,
	  dataType: "json",
	  success: function(data) {
