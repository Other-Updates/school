...
$(".lv_hrs").live('blur',function() 
{
	...
	if($(this).val()=='')
	{
		 $('#texval_'+i).html('Please Enter the Hour'); $('#lvhrs_'+i).focus();
	}
	else
	{
		...
		{
			$('#texval_'+i).html('Given Hour should be less than the Total Hour'); $('#lvhrs_'+i).focus();
		}
		else
		{
			$('#texval_'+i).html(''); 
...
function validatenumber(evt) {
	...
	else{ if(tot_hrs<key){ theEvent.returnValue = false; } }
...
$("#atten_lv_for").live('change',function() //THIS IS FOR GETTING THE DATE LIST FOR COLLEGE LEAVE
{
	...
	else if(vl=='1')
	{
		...
		$.ajax({
			  ...
			  type:'POST',
			  data:{leav_type:vl },
			  success:function(result){ $("#res_div").html(result); for_response('Content Loaded Successfully...!'); }  
			});	
	}
	else if(vl=='2')
	{	
		$("#dys_hrs").html('Hours'); $("#stu_nm").html('Student Name : ');	
...
$("#sub_vl").live('click',function() // AJAX INSERTION FOR COLLEGE LEAVE ALLOCTION FUNCTIONALITY
{
	...
	if(cnt<1){ $('#val_res').html('Please Select Atleat One Date'); return false; }
 ...
 {         
	...
	{			
		$.ajax({
		  ...
		  success:function(result){ /*if(result==1){*/ for_response('Date Saved Successfully...!');  /*location.reload();*/ /*}else{ alert('fail'); }*/ $('#atten_lv_for').trigger("change");$("#sele_cnt").html(''); $('#val_res').html(''); }  
		});	
	}
	else if(atten_for == 2) // this is for student leav
	{
		...
		$.ajax({
		  ...
		  success:function(result){ /*if(result==1){*/ for_response('Date Saved Successfully...!');  /*location.reload();*//*}else{ alert('fail'); }*/$('#date_vl').trigger("change"); $('#lv_type_id').trigger("change");$("#sele_cnt").html('');  }  
		});	
...
$("#lv_type_id").live('change',function()
{
	...
	{
		$('#res_div').html(''); 
		$('.sele_chk').each(function(){ $(this).attr('checked',false); }); 		
	...
	if($("#date_vl").val()!='0')
	{
		...
		if(rol_no!='')
		{
			$.ajax({
			  ...
			  success:function(result){ $("#res_div").html(result); }  
			});		
		}
		else
		{
...
$("#rol_no").live('blur',function()
{
	...
	if($("#rol_no").val()!='')
	{
		$.ajax({
		  ...
		  type:'POST',
		  data:{rol_no : rol_no},
		  success:function(result){ result = result.trim(); if(result=='fail'){ /*alert('Sorry Wrong Roll Number..!')*/}else{ $('#stud_nam').html(result); } }  
		});		
...
$(".sele_chk").live('click',function()
{
	...
	if($(this).attr('checked'))
	{
		  ...
		  $('#lvhrs_'+i).prop('disabled',false); $('#lvhrs_'+i).focus();
	...
	else
	{
		...
		$('#sele_cnt').html(ne_per);
		if(ne_per==''){ $('#sub_vl').prop('disabled',true); }else{ $('#sub_vl').prop('disabled',false); }
...
$('.check_all').live('click',function(){ 
	$('.sele_chk').not(this).prop('checked', this.checked);
	 var v = $('.sele_chk:checked').length;
	  if(v=='0'){ v = ''; $('#sub_vl').prop('disabled',true); }else{ v = v; $('#sub_vl').prop('disabled',false); }
	  $('#sele_cnt').html(v);	
...
<table class="staff_table_sub attendence_style">
  ...
  <tr>
    ...
    <td width="119">
      <?php 
	if(isset($sem_list) && !empty($sem_list)){ ?>	
      ...
      <?php }else{ echo "Oops! Please Add Staffs"; } ?>
    </td>
