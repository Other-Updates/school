...
//				redirect($this->config->item('base_url').'admin/dashboard');
    ...
    function internals() {
        ...
        $input = array('batch_id' => $user_info[0]['batch_id'], 'depart_id' => $user_info[0]['depart_id'], 'group_id' => $user_info[0]['group_id'], 'semester' => $data['all_sem'][0]['id']);
    ...
    function get_all_internals_by_id() {
        ...
        $this->load->model('internal/internal_model');
        $data['ajax_val'] = $this->input->POST();
        $input = array('batch_id' => $user_info[0]['batch_id'], 'depart_id' => $user_info[0]['depart_id'], 'group_id' => $user_info[0]['group_id'], 'semester' => $data['ajax_val']['sem_id']);
        $data['all_grade'] = $this->internal_model->get_all_student_grade($input, $user_info[0]['id']);
        $data['all_subject'] = $this->internal_model->get_all_subject_for_external($input);
        //$where=array('internal.batch_id'=>$user_info[0]['batch_id'],'internal.depart_id'=>$user_info[0]['depart_id'],'internal.group_id'=>$user_info[0]['group_id'],'internal.semester'=>$data['ajax_val']['sem_id']);
        //$data['all_info']=$this->users_model->get_internal_by_std_id($where,$user_info[0]['id']);
