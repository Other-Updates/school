<?php $theme_path = $this->config->item('theme_locations').$this->config->item('active_template'); ?>
            ...
            <table class="staff_table">
                    ...
                    <tr style="visibility:<?=($fees_info[0]['management_amount']==0)?'hidden':''?>;">
                        <td></td>
                        <td>:</td>
                        <td><?=$fees_info[0]['amount']+$fees_info[0]['management_amount']?></td>
                    </tr>
                     <tr>
                        <td></td>
                        <td>:</td>
 ...
 <?php 
						...
						?>
 						...
 						<a href="<?=$this->config->item('base_url').'fees/update_exam_fees_view/'.$fees_info[0]['id'].'/2/hostel_fees'?>" title="Verified" class="btn bg-maroon">
                        	Verified
                        </a>
                        <?php }else if($fees_info[0]['status']==2){?>
                        	<input type="button" disabled="disabled" class="btn bg-maroon" value='Verified'/>
                            <a href="<?=$this->config->item('base_url').'fees/update_exam_fees_view/'.$fees_info[0]['id'].'/3/hostel_fees'?>" title="Closed" class="btn btn-danger">Closed</a>
