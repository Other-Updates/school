...
<table >
	<tr>
        ...
        <td>
        	<input type="text" style="float:left;" name="roll_no" id="roll_update" autocomplete="off" />
            
            <input type="button" style="float:left;margin-left:10px; padding: 4px 10px;" id='view_std' 
            class="btn bg-maroon" value='View' />
...
$().ready(function() {
$('#get_stage_update').autocomplete(BASE_URL+"transport/get_stage_list", {
		width: 260,
		autoFocus: true,
		matchContains: true,
		selectFirst: false
...
<!--getting amount from route-->
$(".route_class").live('change',function()
{
...
<!--getting amount from route-->
$(".route_class_up").live('change',function()
{
	var stage=$('#get_stage_update').val();  
...
$(".bus_class_up").live('change',function()
{
	...
	$.ajax({	
	  url:BASE_URL+"transport/transport_amount_up",
	  type:'get',
	  data:{ 
				bus_id:bus_id, stage:stage,
		   },
...
<script type="text/javascript">
	...
	/*update popup bank detail div show*/
	$(".payment_mode_up").live('change',function()
	{
		var bank=$(this).val();
		if((bank == 3) || (bank == 2 ))  trans_up
		{
			$("#up_bank").show(000);
		}
		if((bank == 1))
	...
	<!--fine amount keyup functional--> 
	$(".fine_class").live('keyup',function()
	{
		var bank=$(this).val();
		var amt_six = $('.s_mon').val()
		if(amt_six != 1)
		{
			if(bank == "")
			{
...
<script>
	...
	$.ajax({	
	  url:BASE_URL+"transport/insert_transp",
	  type:'get',
	  data:{ 
				stage:stage,
				date_from:date_from,
