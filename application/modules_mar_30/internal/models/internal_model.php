...
class Internal_model extends CI_Model {
    ...
    function get_all_cgpa($input, $input1) {
        ...
        $i = 0;
        foreach ($query as $val) {
            $this->db->select('semester,id');
            $this->db->where($input1);
            $query[$i]['semester'] = $this->db->get($this->table_name2)->result_array();
            $j = 0;
            foreach ($query[$i]['semester'] as $val1) {
                $this->db->select('total_cgb');
                $this->db->where('external_id', $val1['id']);
                $this->db->where('std_id', $val['student_id']);
                $this->db->group_by('std_id');
                $query[$i]['semester'][$j]['total_cgp'] = $this->db->get($this->table_name3)->result_array();
                $j++;
            }
            $i++;
        }
