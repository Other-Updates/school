...
?>
<form method="post ">
    <table class="form_table print_admin_use">
        <tr>
            <td>Exam</td>
            <td>&nbsp;</td>
            <td id='td_depart'>
                <select id='exam_id' name='student_group[exam_id]' >
                    <option value="0">Select</option>
                    <?php
                    if (isset($all_exam) && !empty($all_exam)) {
                        foreach ($all_exam as $val) {
                            ?>
                            <option value="<?= $val['id'] ?>"><?= $val['exam'] ?></option>
                            <?php
                        }
                    }
                    ?>
        ...
        <input type="hidden" id="select_sem"  name='external[semester]'  value="<?php echo $batch[0]['id'] ?>" />
    <!--            <td>Batch</td>
                <td>&nbsp;</td>
                <td>
...
//        });
        $('#group_id').live('change', function () {
            ...
            } else
            {
                ...
                $.ajax({
                    url: BASE_URL + "internal/student_report_details",
                    type: 'POST',
                    data: {
                        exam_id: $('#exam_id').val(),
        ...
        $('.grade_option').live('change', function () {
            if ($(this).val() != 'Grade')
            {
                sum = 0;
                avg = 0;
                id_arr = $(this).attr('id').split("_");
                $('#grade_point_' + id_arr[2] + '_' + id_arr[3]).val($(this).val());
                $('#grade_point_total_' + id_arr[2] + '_' + id_arr[3]).val(Number($('#subject_point_' + id_arr[2] + '_' + id_arr[3]).val()) * Number($(this).val()));
                $('.per_' + id_arr[2]).each(function () {
                    sum = sum + Number($(this).val());
                });
                avg = sum / Number($('#subject_grade_point').val());
                $('#total_' + id_arr[2]).val(avg.toFixed(2));
            }
        });
