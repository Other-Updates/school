...
class Exam_model extends CI_Model {
    ...
    function get_all_student_by_exam() {
    ...
    function delete_exam_inactive($id) {
        $this->db->where('id', $id);
    ...
    function delete_exam_inactive_check($id) {
        $query = $this->db->query("select a.id from admin a,staff c where a.exam_id='$id' or c.exam_id='$id'");
        //print_r($query);exit;
        if ($query->num_rows() >= 1) {
            return $query->result_array();
        }
    ...
    function checking_Update($exam, $id) {
        $this->db->select('*');
        $this->db->where('exam', $exam);
        $this->db->where('id !=', $id);
        $this->db->where('df', 0);
