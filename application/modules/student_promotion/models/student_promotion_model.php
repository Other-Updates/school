...
class Student_promotion_model extends CI_Model {
    ...
    function get_all_student_result_cbse($data) {
        $this->db->select('external.*');
        $this->db->select('batch.from,to');
        $this->db->select('department.department');
        $this->db->select('group.group');
        ...
        foreach ($query as $val) {
            ...
            foreach ($query[$i]['external_details'] as $value) {
                ...
                $this->db->join('subject_details', 'subject_details.id=external_details.subject_id');
                $external_details = $this->db->get('external_details')->result_array();
                $count = $this->get_all_subject_for_external($data);
                if (count($count[0]['subject']) == count($external_details))
                    $query[$i]['external_details'][$j]['grade_details'] = $external_details;
                else {
                    $query[$i]['external_details'][$j]['grade_details'] = array();
                }
                $j++;
            }
            $i++;
    ...
    function get_next_section_id($id, $new_group) {
        $this->db->select('id');
        $this->db->where('depart_id >', $id);
        $this->db->where('group', $new_group);
        $this->db->order_by('id');
        $this->db->limit(1);
    ...
    function get_next_group_id($id) {
        $this->db->select('group');
        $this->db->where('id', $id);
        $query = $this->db->get('group');
        if ($query->num_rows() >= 1) {
            $query = $query->result_array();
            return $query[0]['group'];
        }
    }

    function update_external_status($data, $batc_id, $depart_id, $group_id, $semester, $board_id) {
        ...
        $this->db->where('external.board_id', $board_id);
