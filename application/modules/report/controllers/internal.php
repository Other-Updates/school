...
//            exit;
    ...
    public function internal_exam_result() {
        ...
        if (!empty($input_data)) {
            ...
            $baord_id = $input_data['student_group']['board_id'];
            ...
            if (isset($input_data['update_page']) && !empty($input_data['update_page'])) {
                ...
                $input_data['external']['board_id'] = $input_data['student_group']['board_id'];
...
//                print_r($input_data);
    ...
    public function get_all_student_with_subject() {
        ...
        $data['board'] = $g_array = $this->subject_model->get_all_board($input['group_id']);
        $board_id = $g_array[0]['id'];
...
//            exit;
            if ($board_id == 2) {
                echo $this->load->view('internal/update_external_table_cbse', $data);
            } else {
            ...
            if ($board_id == 2) {
                echo $this->load->view('internal/create_external_table_cbse', $data);
            } else {
    ...
    public function external_print($batch_id, $depart_id, $group_id, $semester, $exam_id, $board_id) {
        $this->load->model('internal/internal_model');
        $data['board'] = $g_array = $this->subject_model->get_all_board($group_id);
        if (is_numeric($batch_id) && is_numeric($depart_id) && is_numeric($group_id) && is_numeric($semester) && is_numeric($exam_id)) {
            $input = array('batch_id' => $batch_id, 'depart_id' => $depart_id, 'group_id' => $group_id, 'semester' => $semester, 'exam_id' => $exam_id);
            $data['all_grade'] = $this->internal_model->get_all_student_grade($input);
            $data['all_subject'] = $this->internal_model->get_all_subject_for_external($input);
            $data['post_data'] = $input;
            if ($board_id == 2)
                $this->template->write_view('content', 'internal/print_external_table_cbse', $data);
            else
    ...
    public function student_report_details() {
        ...
        $data['board'] = $g_array = $this->subject_model->get_all_board($post_data['group_id']);
        $board_id = $g_array[0]['id'];
        ...
        if ($board_id == 2)
            echo $this->load->view('internal/print_external_table_cbse', $data);
        else
            echo $this->load->view('internal/print_external_table', $data);
    }

    public function cgpa_report($batc_id, $depart_id, $group_id, $semester, $exam_id) {
        $this->load->model('internal/internal_model');
        $input = array('student_group.batch_id' => $batc_id, 'student_group.depart_id' => $depart_id, 'student_group.group_id' => $group_id);
        $input1 = array('batch_id' => $batc_id, 'depart_id' => $depart_id, 'group_id' => $group_id, 'exam_id' => $exam_id);
        $data['all_cgpa'] = $this->internal_model->get_all_cgpa($input, $input1);
        //$data['all_sem'] = $this->internal_model->get_all_sem($input1);
        $data['all_sem'] = $this->internal_model->get_all_exam($input1);
        $data['get_info'] = $this->internal_model->get_details($input1);
        $data['board'] = $this->subject_model->get_all_board($group_id);
//        echo '<pre>';
//        print_r(  $data['all_cgpa']);
//        exit;
    ...
    function mark_sheet($user_id, $exam_id) {
        ...
        $data['board'] = $this->subject_model->get_all_board($data['student_info'][0]['group_id']);

        $this->template->write_view('content', 'internal/student_mark_sheet', $data);
        $this->template->render();
    }

    function mark_sheet_cbse($user_id, $exam_id) {
        $this->load->model('users/users_model');
        $this->load->model('admin/admin_model');
        $where = array('student_group.student_id' => $user_id);
        $this->load->model('internal/internal_model');
        ...
        $data['board'] = $this->subject_model->get_all_board($data['student_info'][0]['group_id']);
//        echo '<pre>';
//        print_r($data["university_result"]);
//        exit;
        $this->template->write_view('content', 'internal/student_mark_sheet_cbse', $data);
        $this->template->render();
    }

    public function excel($batch_id, $depart_id, $group_id, $semester, $exam_id, $board_id = NULL) {
        $this->load->model('internal/internal_model');
        if (is_numeric($batch_id) && is_numeric($depart_id) && is_numeric($group_id) && is_numeric($semester) && is_numeric($exam_id)) {
            $input = array('batch_id' => $batch_id, 'depart_id' => $depart_id, 'group_id' => $group_id, 'semester' => $semester, 'exam_id' => $exam_id);
            $data['all_grade'] = $this->internal_model->get_all_student_grade($input);
            $data['all_subject'] = $this->internal_model->get_all_subject_for_external($input);
            if ($board_id == 2) {
                $this->load->view('internal/result_excel_cbse', $data);
            } else {
                $this->load->view('internal/result_excel', $data);
            }
        }
    }

    public function pdf($batch_id, $depart_id, $group_id, $semester, $exam_id, $board_id = NULL) {
        ...
        if ($board_id == 2) {
            $body = $this->load->view('internal/result_pdf_cbse', $data, TRUE);
        } else {
            $body = $this->load->view('internal/result_pdf', $data, TRUE);
        }
