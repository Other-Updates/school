...
class Hostel_model extends CI_Model {
    ...
    function get_all_student_room_details() {
        ...
        $this->db->join('student', 'student.std_id=' . $this->table_name10 . '.roll_no');
        $this->db->join('batch', 'batch.id=student.batch_id', 'left');
        $this->db->join('student_group', 'student_group.student_id=student.id');
        $this->db->join('hostel_student_rooms', 'hostel_student_rooms.user_id=student.id', 'left');
        $this->db->join('hostel_rooms', 'hostel_rooms.id=hostel_student_rooms.room_id', 'left');
        $this->db->join('group', 'group.id=student_group.group_id', 'left');
        $this->db->join('department', 'department.id=student_group.depart_id', 'left');
        $this->db->join('master_hostel_block', 'master_hostel_block.id=' . $this->table_name10 . '.block_id', 'left');
        $q = $this->db->get($this->table_name10)->result_array();
    ...
    function get_monthly_report($data) {
        ...
        $this->db->join('master_hostel_block', 'master_hostel_block.id=' . $this->table_name11 . '.block_id', 'left');
        ...
        foreach ($query as $val) {
            ...
            $this->db->join('student', 'student.std_id=hostel_student_advance.roll_no', 'left');
