...
class Student_model extends CI_Model {
    ...
    function get_all_student() {
        ...
        $this->db->join('student_group', 'student_group.student_id=' . $this->table_name . '.id', 'left');
...
//        exit;
    ...
    function is_roll_no_exist($roll_no) {
        $this->db->select($this->table_name . '.id');
        $this->db->where('LCASE(std_id)', strtolower($roll_no));
        $query = $this->db->get($this->table_name);
        if ($query->num_rows() > 0) {
