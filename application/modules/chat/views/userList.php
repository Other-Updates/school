    ...
    <div class="box-body">
        ...
        <div class="my_chat_img">
         <?php $image=$res->image;
										if($image!=""){
										 ?>
                                        <img class="staff_thumbnail"v src="<?= $this->config->item('base_url').'profile_image/staff/thumb/'?><?php echo $res->image; ?>" />
                                        <?php
										}
										else
										{
											?>
							<img class="staff_thumbnail" src="<?= $this->config->item('base_url').'profile_image/staff/thumb/avatar5.png'?>" />	
										<?php
                                        }
										?>
        </div>
        ...
        <p class="message1">
      <?php if($this->user_auth->get_username()==$res->staff_name) { ?>
                   <a href="#" class="name" style="text-decoration:none"><?php echo $res->staff_name; ?></a>
				   <?php } else { ?>  
                   <a href="javascript:void(0)" onClick="javascript:chatWith('<?php echo $res->staff_name; ?>');">
                   <?php } ?>      
...
<table  id="example1" class="table table-striped table-bordered table-hover">
  					...
  					foreach($listOfUsers->result() as $res)
					{
 					?>									
										...
										if($image!=""){
										 ?>
                                        <img class="staff_thumbnail" src="<?= $this->config->item('base_url').'profile_image/staff/thumb/'?><?php echo $res->image; ?>" />
                                        <?php
										}
										else
										{
							...
							<img class="staff_thumbnail" src="<?= $this->config->item('base_url').'profile_image/staff/thumb/avatar5.png'?>" />	
										...
										?>
											...
											<td class="hidden-480"><?php echo $res->staff_name; ?></td>
											<td class="hidden-480">
                                            <style>
                   .green{
				   ...
				   {
				   ?>
                  <div  class="xxx gray icn-only" > </div>
                   <?php } ?>
                   
			   <?php if($this->user_auth->get_username()==$res->staff_name) { ?>
                   <a href="#" style="text-decoration:none"></a>
				   <?php } else { ?>  
                   <a href="javascript:void(0)" onClick="javascript:chatWith('<?php echo $res->staff_name; ?>');" class="btn mini bg-purple btn-sm" ><i class="fa fa-comments-o"></i>
                   <?php } ?>      
                   <?php echo "Chat";  ?>
                                                </a>
