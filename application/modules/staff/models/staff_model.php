...
class Staff_model extends CI_Model {
    ...
    function insert_staff_type($data) {
	if ($this->db->insert('staff_type', $data)) {
	    $id = $this->db->insert_id();
    ...
    function get_all_designation_id_by_name($desig) {
	$this->db->select('id');
	$this->db->where('LCASE(designation)', trim(strtolower($desig)));
	$query = $this->db->get('designation')->result_array();
	//echo "<pre>";print_r($query); exit;
	return $query[0]['id'];
    }

    function get_staff_type_id_by_name($staff_type) {
	$this->db->select('id');
	$this->db->where('LCASE(staff_type)', trim(strtolower($staff_type)));
	$query = $this->db->get('staff_type')->result_array();
	//echo "<pre>";print_r($query); exit;
	return $query[0]['id'];
    }
