...
class Subject_model extends CI_Model {
    ...
    function get_all_subject($user) {
        ...
        $this->db->select('board.board_type');
        ...
        $this->db->join('staff', 'staff.id=' . $this->table_name . '.staff_id', 'left');
        ...
        $this->db->join('board', 'board.id=' . $this->table_name . '.board_id', 'left');
...
//        exit;
    ...
    function get_all_board($id) {
        $this->db->select('board.id,board.board_type');
        $this->db->where($this->table_name2 . '.df', 0);
        $this->db->where($this->table_name2 . '.id', $id);
        $this->db->where($this->table_name2 . '.status', 1);
        $this->db->join('board', 'board.id=' . $this->table_name2 . '.board_id');
        $this->db->group_by($this->table_name2 . '.board_id');
        $query = $this->db->get($this->table_name2);
        if ($query->num_rows() > 0) {
            return $query->result_array();
        }
    ...
    function get_board_id_by_name($staff) {
        $this->db->select('id');
        $this->db->where('LCASE(`board_type`)', trim(strtolower($staff)));
        $query = $this->db->get('board')->result_array();
        if (!empty($query)) {
            return $query[0]['id'];
        }
        return false;
    ...
    function staff_subject($ip) {
        ...
        $this->db->join('board', 'board.id=' . $this->table_name . '.board_id');
