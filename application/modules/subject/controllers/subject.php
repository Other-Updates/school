    ...
    public function insert_subject() {
        ...
        if ($check_practical == 1) {
            $input = array('batch_id' => $this->input->post('value1'), 'subject_name' => $this->input->post('value6'), 'depart_id' => $this->input->post('value2'), 'group_id' => $this->input->post('value3'), 'staff_id' => $this->input->post('value5'), 'cur_year' => $this->input->post('value7'),
                'semester_id' => $this->input->post('value4'), 'scode' => $this->input->post('value8'), 'grade_point' => $this->input->post('grade'), 'nick_name' => $this->input->post('value9'), 'staff_type' => $session_data['staff_type'], 'pass_mark' => $this->input->post('pass_mark'), 'practical_pass_mark' => $this->input->post('practical_pass_mark'), 'practical_mark' => $this->input->post('practical_mark'), 'check_practical' => $this->input->post('check_practical'), 'board_id' => $this->input->post('board'), 'created_user' => $session_data['user_id']);
        } else {
            $input = array('batch_id' => $this->input->post('value1'), 'subject_name' => $this->input->post('value6'), 'depart_id' => $this->input->post('value2'), 'group_id' => $this->input->post('value3'), 'staff_id' => $this->input->post('value5'), 'cur_year' => $this->input->post('value7'),
                'semester_id' => $this->input->post('value4'), 'scode' => $this->input->post('value8'), 'grade_point' => $this->input->post('grade'), 'nick_name' => $this->input->post('value9'), 'staff_type' => $session_data['staff_type'], 'pass_mark' => $this->input->post('pass_mark'), 'board_id' => $this->input->post('board'), 'created_user' => $session_data['user_id']);
        }
...
//        exit;
        ...
        if ($i == 1) {
            ...
            if ($check_practical == 1) {
                $input = array('batch_id' => $this->input->post('value4'), 'subject_name' => $this->input->post('value2'),
                    ...
                    'semester_id' => $this->input->post('value6'), 'scode' => $this->input->post('value8'),
                    'grade_point' => $this->input->post('grade'), 'pass_mark' => $this->input->post('pass_mark'), 'practical_pass_mark' => $this->input->post('practical_pass_mark'), 'practical_mark' => $this->input->post('practical_mark'), 'check_practical' => $this->input->post('check_practical'), 'board_id' => $this->input->post('board'));
            } else {
                $input = array('batch_id' => $this->input->post('value4'), 'subject_name' => $this->input->post('value2'),
                    'depart_id' => $this->input->post('value7'), 'nick_name' => $this->input->post('value9'),
                    'group_id' => $this->input->post('value5'), 'staff_id' => $this->input->post('value3'),
                    'semester_id' => $this->input->post('value6'), 'scode' => $this->input->post('value8'),
                    'grade_point' => $this->input->post('grade'), 'pass_mark' => $this->input->post('pass_mark'), 'check_practical' => $this->input->post('check_practical'), 'board_id' => $this->input->post('board'), 'practical_pass_mark' => NULL, 'practical_mark' => NULL);
            }

            $this->subject_model->update_subject($input, $id);

            if ($this->input->post('board') == 2) {
                $inputs['pass_mark'] = 0;
                $inputs['check_practical'] = 0;
                $inputs['practical_mark'] = NULL;
                $inputs['practical_pass_mark'] = NULL;
                $this->subject_model->update_subject($inputs, $id);
            }
    ...
    public function get_all_board() {
        $this->load->model('subject_model');
        $d_id = $this->input->post();
        $g_array = $this->subject_model->get_all_board($d_id['group_id']);
        $g_select = '<select name="board_id" id="board" class="board_type validate form-control mandatory" >';
        if (isset($g_array) && !empty($g_array)) {
            foreach ($g_array as $val) {
                $g_select = $g_select . "<option value='" . $val['id'] . "'>" . $val['board_type'] . "</option>";
            }
        }
        $g_select = $g_select . '</select>';
        echo $g_select;
    ...
    public function get_all_staff1() {
        ...
        $d_id = $this->input->post();
        $g_array = $this->subject_model->get_all_staff($d_id['depart_id']);
        $g_select = '<select id="staff1" name="student_staff" class="u_group1 mandatory validate dupe1" ><option value="">Select Staff</option>';
        if (isset($g_array) && !empty($g_array)) {
            foreach ($g_array as $val) {
                $g_select = $g_select . "<option value='" . $val['id'] . "'>" . $val['staff_name'] . "</option>";
            }
    ...
    public function get_all_bd() {
        ...
        $data['all_semester'] = $this->subject_model->semester();
        $d_id = $this->input->post();
        $g_array = $this->subject_model->get_all_board($d_id['grp_id']);

        $g_select = '<select id="board" name="student_group[board_id]" class="mandatory u_val u_board" >';
        if (isset($g_array) && !empty($g_array)) {
            foreach ($g_array as $val) {
                $g_select = $g_select . "<option value='" . $val['id'] . "'>" . $val['board_type'] . "</option>";
            }
    ...
    function import_subject() {
        ...
        if ($this->input->post()) {
            ...
            if (!empty($_FILES['subject_data'])) {
                ...
                $file = FCPATH . 'attachement/subject/' . $file_name;
                $handle = fopen($file, 'r');
                $skip_rows = 1;
                ...
                if ($file != NULL) {
                    while ($row_data = fgetcsv($handle)) {
                        ...
                        if (empty($staff_id)) {
                            ...
                            $staff_id = $this->subject_model->insert_staff($staff_details);
                            // $staff_id = $stf_id['id'];
                        }

                        $board_id = $this->subject_model->get_board_id_by_name($board);
                        if (empty($board_id)) {
                            $b_details = array(
                                'board_type' => $board,
                                'ldt' => date('Y-m-d H:i:s'),
                            ...
                            $board_id = $this->subject_model->insert_board($b_details);
                            // $board_id = $b_id['id'];
                        }
